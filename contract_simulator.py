...
# 智能操作金额逻辑说明
with st.expander("🧠 智能批量操作金额逻辑", expanded=False):
    st.markdown("""
    ### 📊 各操作的金额计算逻辑：
    
    #### 💰 **Deposit 操作** (Deposit O1 / Deposit O2)
    - **金额范围**: 交易账户余额的 **5%-20%**
    - **最小金额**: 至少 1.0 USDC 或余额的5%（取较小值）
    - **计算公式**: `random.uniform(min_amount, max_amount)`
    - **示例**: 交易账户有100 USDC → 操作金额 5-20 USDC
    
    #### 🏦 **Add Liquidity 操作**
    - **金额范围**: LP提供者账户余额的 **5%-15%**
    - **最小金额**: 至少 1.0 USDC 或余额的5%（取较小值）
    - **计算公式**: `random.uniform(min_amount, max_amount)`
    - **示例**: LP提供者有200 USDC → 操作金额 10-30 USDC
    
    #### 🔄 **Remove Liquidity 操作**
    - **金额范围**: 基于LP代币余额，最多移除一半
    - **保护机制**: 至少保留 0.1 个LP代币
    - **最大可移除**: `(LP代币余额 - 0.1) × 0.5`
    - **最小金额**: 0.1 USDC
    - **示例**: 有10个LP代币 → 最多可移除 4.95个
    
    #### 📤 **Withdraw 操作** (Withdraw O1 / Withdraw O2)
    - **金额范围**: 基于对应代币余额，最多卖出一半
    - **保护机制**: 至少保留 0.1 个代币
    - **最大可卖出**: `(代币余额 - 0.1) × 0.5`
    - **最小金额**: 0.1 USDC
    - **示例**: 有8个O1代币 → 最多可卖出 3.95个
